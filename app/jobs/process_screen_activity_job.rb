    ...
    begin
      ...
      enterScreen.each do |row|
        if not_test_conditions(row['test'], row['version'])
          ...
          if days <= 0
            puts "#{days} = #{u_device.first_play_date.inspect} - #{keen_date.inspect}"
            puts "#{u_device.first_play_date.to_datetime} vs #{keen_date}"
          end

          u_s_d = UserScreenDay.find_by(day: days, screen_name: row['screenName'])
          if u_s_d
            # update_c = update_c + 1
            u_s_d.access_count = u_s_d.access_count + 1
            u_s_d.save!
          else
            # new_c = new_c + 1
            u_s_d = UserScreenDay.new
            u_s_d.day = days
            u_s_d.screen_name = row['screenName']
            u_s_d.access_count = 1
            u_s_d.save!
          end
        else
          # puts "REJECTED: T:[#{row['test'].inspect}] V:[#{row['version'].inspect}], #{row['test'].nil?}, #{row['test'].is_a?(FalseClass)}, #{row['test'].is_a?(String)}"
